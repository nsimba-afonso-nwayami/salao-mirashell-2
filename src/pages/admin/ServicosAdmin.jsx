...
export default function ServicosAdmin() {
  ...
  // Ordenação agora só por Nome ou Preço
  const [ordenacao, setOrdenacao] = useState("nome_asc"); // nome_asc, preco_desc

  // DADOS FICTÍCIOS DE SERVIÇOS (mantive a duração no objeto, mas não será exibida)
  const todosServicos = [
  ...
  const handleAdd = () => {
    alert("Ação: Abrir formulário para Novo Serviço");
  };
  ...
  return (
    <div className="min-h-screen bg-stone-950 text-white flex">
      ...
      {/* SIDEBAR (Reutilizado) - Sem Alterações */}
      <aside
        className={`
      ...
      <div className="flex-1 md:ml-64 overflow-y-auto"> 
        {/* HEADER (Reutilizado) - Sem Alterações */}
        <header
          className="
        ...
        {/* CONTEÚDO PRINCIPAL: TABELA DE SERVIÇOS */}
        <main className="mt-20 p-4 sm:p-6 space-y-8 sm:space-y-10">
          ...
          <div className="bg-stone-900/30 border border-stone-800 rounded-xl shadow-xl backdrop-blur-md p-4 sm:p-6">
            <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
                <h3 className="text-2xl font-bold text-amber-400">Serviços Oferecidos ({servicosProcessados.length})</h3>
                <button 
                    onClick={handleAdd}
              ...
              <table className="min-w-full w-full text-left border-collapse text-sm"> 
                <thead>
                  <tr className="text-amber-300 border-b border-stone-800/70">
                    ...
                    <th className="p-3 whitespace-nowrap">Nome do Serviço</th>
                    <th className="p-3 whitespace-nowrap">Categoria</th>
                    <th className="p-3 whitespace-nowrap text-right">Preço</th>
                    {/* COLUNA DE DURAÇÃO REMOVIDA */}
                ...
                <tbody>
                  {servicosProcessados.length > 0 ? (
                    servicosProcessados.map((servico) => (
                      ...
                      >
                        <td className="p-3 opacity-70 whitespace-nowrap">{servico.id}</td>
                        <td className="p-3 font-medium whitespace-nowrap">{servico.nome}</td>
                        <td className="p-3 whitespace-nowrap opacity-90">{servico.categoria}</td>
                        <td className="p-3 whitespace-nowrap font-bold text-amber-300 text-right">{formatarPreco(servico.preco)}</td>
                        {/* CÉLULA DE DURAÇÃO REMOVIDA */}
                        <td className="p-3 text-center whitespace-nowrap">
                          <span className={`px-3 py-1 rounded-lg text-xs font-semibold ${
                            servico.ativo ? 'bg-green-600/30 text-green-300' : 'bg-red-600/30 text-red-300'
                          }`}>
            ...
            <div className="flex justify-between items-center mt-6">
              <p className="opacity-70 text-sm">
                Mostrando {servicosProcessados.length} de {todosServicos.length} Serviços
              </p>
            </div>
